...
export async function GET(request: Request) {
  ...
  // 'upcoming' and 'past' are stored as 'upcoming' in the database
  // and distinguished by whether the consultation time has passed.
  ...
  const { data, error, count } = await query.order('datetime', { ascending: false }).range(from, from + pageSize - 1);
  ...
  return NextResponse.json({ consultations: rows.map(toConsultation), total: count ?? 0 }, { status: HTTP_STATUS.OK });
}
